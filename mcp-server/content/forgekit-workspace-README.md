...
If `.forgekit/` was gitignored, it is not in git. Copy from upstream Catalyst Forge (`mcp-server/content/`) or restore from backup; MCP-only projects can recreate tracking via **`getInitialWorkflowTracking`**.
