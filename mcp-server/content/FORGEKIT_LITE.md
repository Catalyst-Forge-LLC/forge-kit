...
- **Upstream template:** `forge-kit/mcp-server/content/FORGEKIT_LITE_UPDATES.md` (empty starter — copy into `.forgekit/`).
...
| `scripts/forgekit-dev-launcher.mjs`, `scripts/setup-pocketbase.mjs`, `scripts/test-*.mjs`, `scripts/setup-ollama.mjs` | **Agent** (from upstream `mcp-server/content/scripts/`) | Phase 2 |
...
2. **Default:** resolve **latest stable** at **install time** (GitHub `pocketbase/pocketbase` releases API). Copy upstream **`mcp-server/content/scripts/setup-pocketbase.mjs`** into the app repo (or equivalent in `scripts/setup-pocketbase.ts`).
...
1. Copy upstream **`forgekit-dev-launcher.mjs`** and **`setup-pocketbase.mjs`** from `mcp-server/content/scripts/` into the app **`scripts/`** folder (adapt paths if the app uses `app/`).
...
Save upstream **`mcp-server/content/cursor-rules/forgekit-no-trailer.mdc`** as **`.forgekit/cursor/rules/forgekit-no-trailer.mdc`**. Symlink or copy into **`.cursor/rules/`** (§1.5). Also create **`.forgekit/AGENTS.md`** and **`.forgekit/CLAUDE.md`** with the same policy (§12 snippets) — users switch tools between sessions.
...
Save as **`.forgekit/cursor/rules/forgekit-updates-log.mdc`**. Symlink or copy into **`.cursor/rules/`** (§1.5). Use the file at `mcp-server/content/cursor-rules/forgekit-updates-log.mdc` in the ForgeKit repo as the canonical upstream text — keep it in sync when §1.6 changes.
