# Using Native Plan Modes with ForgeKit

When your host agent supports a first-class plan mode (Grok Build `/plan` + enter/exit_plan_mode, or equivalents), use it as the implementation of ForgeKit **Phase 1 (Architecture + Planning)**.
...
- `.forgekit/workflow_tracking.json` with `currentPhase: "1-architecture"` (or advancing), `decisions[]` populated, and exit criteria updated
