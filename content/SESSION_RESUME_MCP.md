# ForgeKit — Resume session (MCP-first)

The user is **continuing** a project that uses ForgeKit via the **MCP server** only. There is **no** `_forgekit/` folder in the repo.

---

## Before doing anything else

1. Read **`.forgekit/workflow_tracking.json`** — current phase, exit criteria, notes, decisions.
...
3. Call **`getPhaseGuidance`** for the phase matching `.forgekit/workflow_tracking.json` → `currentPhase` (e.g. `4` for feature iteration, or keyword `features`).
...
Do **not** require `_forgekit/WORKFLOW.md` or `_forgekit/TRACKING_SCHEMA.md` on disk — use the MCP tools above.
...
- After work, **update `.forgekit/workflow_tracking.json`** (criteria, `decisions`, `gotchas`, `sessions`) per **`getTrackingSchema`**.
...
- If the repo has **gstack** installed, use its skills for sprint execution (build/review/qa/ship) within ForgeKit phases — see **WORKFLOW.md §1b** or call **`getPhaseGuidance`** for phase-specific gstack integration. After each gstack sprint, update `.forgekit/workflow_tracking.json`.
