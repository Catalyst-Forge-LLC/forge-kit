...
You are helping me build a full-stack web application. I use ForgeKit, a structured phase-based development workflow refined across multiple projects. Your job is to follow this workflow, track progress, and pause at phase transitions for my approval before advancing.

---

### MCP-first (recommended — no `_forgekit/` copy in this repo)

If **ForgeKit is connected as an MCP server**, do **not** require a local `_forgekit/` folder. Methodology, templates, and audits stay in the MCP server (open-source ForgeKit install); persist only **`.forgekit/workflow_tracking.json`** and optional Cursor rules in this repo.
...
**Replies to me:** When presenting several options or a default order of work, follow ForgeKit **`USER_REPLY_FORMAT.md`** (or MCP **`getUserReplyFormat`**): **numbered** lists for pipelines, **bullets** for parallel items, **letters** for pick-one—don’t bury multiple paths in one long sentence.
...
**Phase 1 — state persistence (web apps):** before locking PocketBase + auth, ask me *"Does any state need to outlive this browser — accounts, cross-device sync, shared data — or is per-user state fine in `localStorage`?"* If **local-only**: drop PocketBase + auth, target `adapter-static`, no deploy-time secrets. If **persistent**: full Default-A stack. Record the answer in **`PHASE_1_BRIEF.md` §4 (`State persistence:`)** and **`decisions[]`**. See **`FORGEKIT_LITE.md` §7** (A-local vs A-persistent).
...
Record pattern, provider (e.g. `ollama/ibm/granite4.1:8b`), env var names, paths, and validator in **`PHASE_1_BRIEF.md`** (content-generation section) and **`decisions[]`**. See **`FORGEKIT_LITE.md` §7.1** for OpenAI, Ollama, seed, and BYO skeletons.

**External listing/article URLs (if the hero flow imports from pasted URLs):** follow **`FORGEKIT_LITE.md` §7.2** — layered fetch and parse, **don’t blame users** for correct URLs when selectors rot, optional **single** structured **verbatim** recover from stripped page text (env-gated). Expand into **`docs/TECHNICAL_REFERENCE.md`** using the ForgeKit template subsection on deterministic extractors vs markup drift.

**Git commits:** use `git commit -F <file>` or plain `-m`; no unrequested attribution trailers (`Made-with:`, `Co-Authored-By:`, etc.). **Git 2.32.0+** (2021) supports `--trailer` natively — not a compatibility problem on current Git. **Pre-2.32 only:** if commit fails with `unknown option 'trailer'`, use `bash -c "git commit -F path/to/msg.txt"` or upgrade Git. See **`FORGEKIT_LITE.md` §8.9**.

Skip the **“copy-paste paths”** section below unless we are using a **local** ForgeKit folder.

---

### Local `_forgekit/` folder (template-in-repo mode)

> **Path note:** ForgeKit files live in `_forgekit/` by default. If you placed the folder elsewhere (e.g., a sibling `forge-kit/` directory), adjust paths accordingly.
...
- At the start of each session, read `.forgekit/workflow_tracking.json` and `CONTEXT_PROMPT.md` (if it exists).
...
- After completing work, update `.forgekit/workflow_tracking.json` following the structure in `TRACKING_SCHEMA.md`: move satisfied exit criteria, add decisions to the `decisions` array (with rationale), log issues to the `gotchas` array, and add session notes.
...
- Create **`docs/PHASE_1_BRIEF.md`** from the ForgeKit template (`_forgekit/docs/` or MCP **`getTemplate("PHASE_1_BRIEF")`**). Fill and **lock** it before we exit Phase 1.
- Record major commitments in **`.forgekit/workflow_tracking.json`** → **`decisions[]`** (and phase notes) so the next session does not depend on chat history.

Phase 2 (Scaffolding + Core Build):

- Only after I've confirmed the architecture and the brief is locked.
- Read **`PHASE_1_BRIEF.md`** and **`.forgekit/workflow_tracking.json`** first.
...
- **Documentation (progressive — see WORKFLOW.md §1a or ForgeKit MCP `getProgressiveDocSchedule`):** In Phase 2, create **only** these four from ForgeKit templates — **either** copy from `_forgekit/docs/` **or** use MCP **`getTemplate`** (`CONTEXT_PROMPT`, `README`, `TODO`, `IDEAS`):
