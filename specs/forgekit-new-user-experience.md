...
**Status:** Draft — not started. Captures author intent from a fresh-eyes README review (2026-07-10). Living until work is sliced into partial/completed specs or absorbed into README + Lite + Genesis packaging.
...
- [`content/NEW_PROJECT_BOOTSTRAP.md`](../../content/NEW_PROJECT_BOOTSTRAP.md) (MCP kickoff; step 0 already mentions Genesis)
...
- **G2.** A single **recommended first path** that proves value with zero MCP / zero global CLI: Genesis prompt → `GENESIS.md` → folder with Lite → coding agent kickoff.
...
- Changing install CLI behavior beyond what packaging the try-it path needs (e.g. a `try/` or `starter/` folder of static files is in scope; a new SaaS signup is not).
...
1. User pastes the Genesis prompt into their favorite AI chat (ChatGPT, Claude, Grok, Ollama UI, …).
2. They iterate until they have a solid `GENESIS.md` (what, not how).
3. They create an empty project folder, add `docs/GENESIS.md` (or root `GENESIS.md`) and `.forgekit/FORGEKIT_LITE.md` (or paste Lite / run `forgekit install --lite` if they already have the CLI).
4. They open that folder in their coding agent and say a short kickoff line (to be standardized in this work).
...
- **Try path** — Genesis + Lite, no MCP, no global link required if the user can download or copy two files.
...
1.1 Opening must answer, in order: **what** (one sentence), **who** (solo / small-team builders using AI coding agents), **why** (phases + memory + lessons so the agent does not thrash), **how to try** (link or inline to Try path).
...
1.4 Repo GitHub description / topics (if editable in-repo docs) should match the one-sentence what.

### 5.2 Try path: Genesis + Lite

2.1 Document a numbered human recipe (≤10 steps) that does not require `pnpm`, MCP, or `forgekit` on PATH.
2.2 Provide a **downloadable or copyable** pair: Genesis prompt text + Lite file (or clear links to raw GitHub / `content/` paths).
2.3 Provide a **canonical kickoff line** for the coding agent, e.g. shape: *Follow `.forgekit/FORGEKIT_LITE.md`. Treat `docs/GENESIS.md` as the product spec. Ask intake only for gaps. Do not write app code until I approve the Phase 1 brief.*
...
3.1 Genesis prompt remains available via MCP (`getGenesisSpecPrompt`) **and** as a first-class human artifact (README link + optional `TRY.md` or `docs/TRY_FORGEKIT.md` in the forge-kit repo).
3.2 Lite install docs mention "pair with a Genesis spec" as the preferred greenfield when the user already has a written idea.
3.3 MCP bootstrap / kickoff continues to offer Genesis as step 0 when the user has only an idea (already partially present) — keep aligned with README language.
3.4 Optional: a `try/` or `starters/genesis-lite/` folder with README + empty `docs/GENESIS.md` placeholder + pointer to Lite — only if it reduces friction without duplicating Lite content.
...
5.1 Same Try narrative in README, Lite §1 (human skim), and Genesis "After you have GENESIS.md" (already agent-oriented — add a human-oriented sibling paragraph).
...
- **Maintenance:** Try path docs must not fork Lite or Genesis content; link or thin wrappers only.
...
- User has **no idea yet** — Try path should still offer "use Genesis with a vague one-liner" or "skip Genesis and use Lite §5 intake only," without forcing market research.
- User already has a **long PRD / Notion dump** — map to "treat as Genesis; optionally run Genesis prompt only for gaps / prior-art / format research."
...
|-----------|---------|----------------|
| **M1** | README rewrite: hero + Try path (Genesis+Lite) + ranked Quickstart; MCP/full demoted but linked | Docs only |
| **M2** | Canonical kickoff line + short `TRY.md` (or equivalent) with the 15-minute recipe; cross-links from Genesis + Lite §1 | Docs only |
| **M3** | Optional `try/` starter folder or GitHub "Use this template" notes; raw file links verified | Light packaging |
| **M4** | Align MCP kickoff / bootstrap wording with README Try narrative; smoke-test with a cold reader or second machine | Docs + small content sync |
| **M5** | (Later) Sample anonymized `GENESIS.md` excerpt or link to a public example project that used the path | Optional social proof |

---

## 9. Acceptance criteria

- **AC1.** Given a reader who has never heard of ForgeKit, when they read only the first screenful of the README, then they can state what it is, who it is for, and the recommended first action without scrolling past the Try section.
- **AC2.** Given a user with an idea and no Node/MCP setup, when they follow the Try recipe, then they can place Genesis + Lite in a new folder and paste a single kickoff line into their coding agent without running `mcp:build` or `link:global`.
- **AC3.** Given that kickoff, when the agent follows Lite, then it does not scaffold app code before Phase 1 brief approval (or the Try doc explicitly tells the user to stop the agent if it does).
...
- **AC5.** Given maintainers update Genesis or Lite, when they follow existing sync rules, then the Try recipe does not require maintaining a third full copy of either document.
...
| # | Question | Notes |
|---|----------|-------|
| Q1 | Name the human doc `TRY.md`, `docs/TRY_FORGEKIT.md`, or fold entirely into README? | Prefer one durable URL for sharing |
| Q2 | Should GitHub "About" / topics be updated in the same PR as M1? | Out of markdown; still part of NUX |
| Q3 | Include a minimal sample `GENESIS.md` (fictional tool) in-repo, or only the prompt? | Sample helps; may go stale |
| Q4 | Does `forgekit install --lite` gain a `--with-genesis-stub` flag, or stay docs-only for M1–M2? | Flag is convenience; not required for AC2 |
| Q5 | Should the Try path recommend saving Genesis at `docs/GENESIS.md` vs repo root? | Align with ingestPlanArtifact / Lite intake |
| Q6 | Measure success qualitatively (author dogfood) or with a short checklist issue template for first-time reporters? | Optional |
...
- Prefer linking to `content/GENESIS_SPEC_PROMPT.md` and `content/FORGEKIT_LITE.md` over duplicating their bodies in the README.

---

## 12. Implementation notes (light — not a build plan)

When work starts, prefer **M1 → M2** as a single docs PR if small enough; move this file to `specs/partial/` then `specs/completed/` per WORKFLOW spec lifecycle. Update [`specs/README.md`](README.md) index when status changes. If this becomes a standing roadmap, promote a slim version to `specs/canonical/` and keep delivery criteria in completed/.

**Suggested kickoff line (draft for M2 — edit freely):**
