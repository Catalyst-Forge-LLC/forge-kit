# ForgeTrail — New user experience (NUX) improvement

**Spec kind:** Feature delivery (ForgeTrail product / docs)

**Status:** Completed 2026-07-10 — M1–M4 shipped. M5 (link a real public project) deferred as optional social proof.

**Scope:** How a **first-time visitor** understands what ForgeTrail is, why it is worth trying, and how to prove it out in under an hour — without cloning MCP, linking a global CLI, or reading the full methodology tree. Primary prove-it path: **Genesis spec prompt + ForgeTrail Lite**. Secondary paths (MCP, full install) stay available but must not dominate the front door.

**Related artifacts:**
- Root [`README.md`](../../README.md) (Try-first front door)
- Root [`TRY_FORGETRAIL.md`](../../TRY_FORGETRAIL.md) (human try recipe — locked name)
...
- [`content/FORGETRAIL_LITE.md`](../../content/FORGETRAIL_LITE.md)
- [`content/NEW_PROJECT_BOOTSTRAP.md`](../../content/NEW_PROJECT_BOOTSTRAP.md)
- [`.github/ISSUE_TEMPLATE/try-forgetrail-checklist.md`](../../.github/ISSUE_TEMPLATE/try-forgetrail-checklist.md)
- [`mcp-server/README.md`](../../mcp-server/README.md)
- [`specs/canonical/forgetrail-modern-agents-evolution.md`](../canonical/forgetrail-modern-agents-evolution.md)

---

## 1. Problem statement

A fresh user landing on the ForgeTrail README today faces:
...
3. **The easiest prove-it path is buried.** Genesis (paste a prompt in any chat app → iterate a `GENESIS.md`) and Lite (one markdown file + a coding agent) are the lowest-friction way to feel ForgeTrail without Node MCP setup. They are not the hero of the README.
...
**What "done" looks like for this initiative:** A stranger can open the README, understand ForgeTrail in ~30 seconds, pick a **Try it in 15 minutes** path that uses Genesis + Lite (no MCP required), and know when to graduate to MCP or full install — without reading WORKFLOW.md first.
...
- **G1.** Front-door clarity: what ForgeTrail is, who it is for, and the one outcome it sells (disciplined AI-assisted builds with memory across sessions).
...
- **G4.** Clear **graduation ladder**: Try (Genesis+Lite) → Prefer Lite for small tools → Prefer MCP when living in Cursor/Claude with ForgeTrail cloned → Full `_forgetrail/` only when offline/vendored methodology is required.
...
- Building a hosted/cloud ForgeTrail product or interactive web onboarding.
...
| **`FORGETRAIL_LITE.md`** | Single-file protocol; `forgetrail install --lite` | Excellent for agents; human still needs to know *to* drop it next to a genesis spec |
| **`getNewProjectKickoff` / MCP** | Best in-IDE experience once configured | High setup cost for "just trying" |
| **Full install / `_forgetrail/`** | Self-contained methodology tree | Heavy for first proof; competes with Lite for attention |
...
3. They create an empty project folder, add `docs/GENESIS.md` and `.forgetrail/FORGETRAIL_LITE.md` (copy files, or `forgetrail install --lite --with-genesis-stub` if they have the CLI).
4. They open that folder in their coding agent and paste the kickoff line from `TRY_FORGETRAIL.md`.
...
- **Try path** — Genesis + Lite, no MCP, no global link required if the user can download or copy two files. Documented in **`TRY_FORGETRAIL.md`**.
...
1.1 Opening must answer, in order: **what** (one sentence), **who** (solo / small-team builders using AI coding agents), **why** (phases + memory + lessons so the agent does not thrash), **how to try** (link to `TRY_FORGETRAIL.md`).
1.2 "Compounding loop," comparison table, gstack, PocketBase/Ollama optionals, and "what's in the box" tree must not appear before the Try path (or must be collapsed behind a short "Why ForgeTrail" / "Deeper" section).
...
2.1 Document a numbered human recipe in **`TRY_FORGETRAIL.md`** (≤10 steps) that does not require `pnpm`, MCP, or `forgetrail` on PATH.
...
2.3 Provide a **canonical kickoff line** in `TRY_FORGETRAIL.md` for the coding agent.
2.4 State success criteria for the try: e.g. `.forgetrail/workflow_tracking.json` exists, `docs/PHASE_1_BRIEF.md` drafted from Genesis, user asked for approval before scaffold.
...
3.1 Genesis prompt remains available via MCP (`getGenesisSpecPrompt`) **and** as a first-class human artifact (README link + `TRY_FORGETRAIL.md`).
3.2 Lite install supports **`--with-genesis-stub`**: creates `docs/GENESIS.md` stub (pointer to prompt + sample) alongside `.forgetrail/` Lite files.
...
4.2 Document when full `_forgetrail/` install is warranted (air-gapped, no MCP, want full template tree on disk).
4.3 Do not present all three as equal "Quickstart" bullets without ranking.

### 5.5 Messaging consistency

5.1 Same Try narrative in README, `TRY_FORGETRAIL.md`, Lite §1 (human skim), and Genesis "After you have GENESIS.md".
5.2 Avoid jargon in the first screenful (no "stdio," "FORGETRAIL_ROOT," "pnpm link" until MCP section).
...
- **Tone:** Outcome-oriented US English; assertive capability copy for what ForgeTrail does; no em dashes in user-facing README/Try copy (per project rules).
...
- **Windows / Git Bash PATH** issues with `forgetrail` CLI — Try path must not depend on global link (copy files or raw GitHub URLs).
...
- User tries Try path **inside the forgetrail clone** by mistake — warn: use a **new empty project folder**, not the methodology repo.
...
|-----------|---------|----------------|
| **M1** | README rewrite: hero + link to `TRY_FORGETRAIL.md` + ranked Quickstart; MCP/full demoted but linked; GitHub About/topics updated in same PR | Docs + repo metadata |
| **M2** | `TRY_FORGETRAIL.md` with 15-minute recipe + kickoff line; cross-links from Genesis + Lite §1 | Docs |
| **M3** | Mars sample Genesis + `forgetrail install --lite --with-genesis-stub`; raw file links verified | Packaging |
...
- **AC1.** Given a reader who has never heard of ForgeTrail, when they read only the first screenful of the README, then they can state what it is, who it is for, and the recommended first action (`TRY_FORGETRAIL.md`) without scrolling past the Try section.
- **AC2.** Given a user with an idea and no Node/MCP setup, when they follow `TRY_FORGETRAIL.md`, then they can place `docs/GENESIS.md` + Lite in a new folder and paste a single kickoff line into their coding agent without running `mcp:build` or `link:global`.
- **AC3.** Given that kickoff, when the agent follows Lite, then it does not scaffold app code before Phase 1 brief approval (or `TRY_FORGETRAIL.md` explicitly tells the user to stop the agent if it does).
...
- **AC5.** Given maintainers update Genesis or Lite, when they follow existing sync rules, then `TRY_FORGETRAIL.md` does not require maintaining a third full copy of either document.
...
- **AC7.** Given `forgetrail install --lite --with-genesis-stub`, when the target has no `docs/GENESIS.md`, then a stub is created at that path; existing files are left alone unless `--force`.
...
| # | Decision | Detail |
|---|----------|--------|
| **D1 (Q1)** | Human try doc is **`TRY_FORGETRAIL.md`** at repo root | Filename itself reminds users what it is; durable share URL next to README |
| **D2 (Q2)** | Update GitHub About / topics in the **same PR as M1** | Description (draft): *Structured 7-phase methodology for solo builders using AI coding agents. Try without MCP: Genesis spec + ForgeTrail Lite.* Topics (draft): `ai-agents`, `methodology`, `cursor`, `mcp`, `spec-driven-development`, `solo-developer` |
...
| **D4 (Q4)** | **`forgetrail install --lite --with-genesis-stub`** | Creates `docs/GENESIS.md` stub (does not overwrite without `--force`) |
| **D5 (Q5)** | Canonical Genesis path is **`docs/GENESIS.md`** | Aligns kickoff line, ingestPlanArtifact guidance, and Lite intake |
| **D6 (Q6)** | Short **GitHub issue checklist** for first-timers | `.github/ISSUE_TEMPLATE/try-forgetrail-checklist.md` |
...
- Do not weaken MCP as the recommended path **after** the user has cloned ForgeTrail and lives in Cursor — only reorder first contact.
- User-facing copy: US English; no em dashes; assertive product capability language where describing shipped behavior.
- Prefer linking to `content/GENESIS_SPEC_PROMPT.md` and `content/FORGETRAIL_LITE.md` over duplicating their bodies in the README or `TRY_FORGETRAIL.md`.
...
**Canonical kickoff line (locked for `TRY_FORGETRAIL.md`):**
...
| **M2** | `TRY_FORGETRAIL.md` + kickoff line; Lite §1 human pointer | Done |
...
| AC2 | TRY_FORGETRAIL.md recipe needs no MCP/CLI |
...
| AC8 | `.github/ISSUE_TEMPLATE/try-forgetrail-checklist.md` |

**Key paths:** `TRY_FORGETRAIL.md`, `README.md`, `content/GENESIS_SPEC_PROMPT.md`, `content/GENESIS_STUB.md`, `content/examples/GENESIS_SAMPLE_mars-habitat-roster.md`, `content/NEW_PROJECT_BOOTSTRAP.md`, `content/FORGETRAIL_LITE.md` §1, `INITIAL_PROMPT.md`, `mcp-server/README.md`, `mcp-server/src/index.ts` kickoff intro.
